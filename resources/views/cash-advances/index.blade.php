...
@section('content')
    <div class="bg-white rounded-lg border border-neutral-200">
        <div class="flex flex-col md:flex-row items-center justify-between p-4 gap-4">
            <form method="GET" action="{{ route('cash-advances.index') }}" class="relative w-full max-w-md">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari Pegawai..." class="w-full bg-neutral-50 border border-neutral-200 text-sm rounded-lg pl-9 pr-4 py-2 focus:outline-none focus:border-[#347839] focus:bg-white">
            </form>

            <a href="{{ route('cash-advances.create') }}" class="w-full md:w-fit text-center bg-[#347839] text-white rounded-lg px-4 py-2.5 text-sm font-medium hover:bg-[#22552d] transition-colors duration-200">
                Tambah Kasbon
            </a>
        </div>

        <div class="w-full overflow-auto">
            <x-table :headers="['Nama Pegawai', 'Tanggal', 'Jumlah', 'Sisa Tagihan', 'Aksi']" :data="$cashAdvances">
                @foreach ($cashAdvances as $item)
                    <tr class="cursor-pointer even:bg-neutral-100 hover:bg-neutral-50 transition-colors duration-200">
                        ...
                        <td class="px-4 py-2.5 text-neutral-500">Rp{{ number_format($item->remaining_amount, 0, ',', '.') }}</td>
