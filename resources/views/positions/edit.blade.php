        ...
        <form method="POST" action="{{ route('positions.update', $position->id) }}" class="space-y-4">
            ...
            <div>
                ...
                <input
                    ...
                    value="{{ old('sales_target', $position->sales_target) }}"
                    class="w-full border text-sm bg-neutral-50 px-4 py-2 rounded-lg focus:outline-none active:bg-white
            ...
            <div x-data="{
                rules: {{ json_encode(old('bonusRules', $position->bonusRules->map(fn($r) => [
            ...
            <div x-data="{
                bonuses: {{ json_encode(old('performanceBonuses', $position->performanceBonuses->map(fn($b) => [
                    'name'   => $b->name,
                    'amount' => $b->amount,
                ])->toArray())) }},
                errors: {{ json_encode(
            ...
            <div x-data="{
                penalties: {{ json_encode(old('penalties', $position->penalties->map(fn($p) => [
                    'name'  => $p->name,
                    'type'  => $p->type,
                    'value' => $p->value,
                ])->toArray())) }},
